...
from panorama_consts import (
    ...
    PAN_JSON_DEVICE_GRP,
    PAN_JSON_DST,
    PAN_JSON_POLICY_NAME,
    PAN_JSON_POLICY_TYPE,
    PAN_JSON_WHERE,
    POLICY_TYPE_VALUE_LIST,
...
class MovePolicy(BaseAction):
    def execute(self, connector):
        ...
        if where and where not in ["after", "before", "top", "bottom"]:
            return action_result.set_status(
                phantom.APP_ERROR, VALUE_LIST_VALIDATION_MESSAGE.format(["after", "before", "top", "bottom"], PAN_JSON_WHERE)
            )
        ...
        if dst_device_group != curr_device_group or dst_pre_post != curr_pre_post:
            param = {PAN_JSON_DEVICE_GRP: dst_device_group}
            ...
            data.update({"type": "config", "action": "multi-move", "xpath": xpath, "element": element})
            if where:
            ...
            if len(policy_list) > 1:
                return action_result.set_status(
                    phantom.APP_ERROR,
                    "Moving multiple policies at a time is only possible if they have \
                                                to be moved to a different device group or rule base",
                )
            param = {PAN_JSON_DEVICE_GRP: curr_device_group, PAN_JSON_POLICY_NAME: policy_name, PAN_JSON_POLICY_TYPE: curr_pre_post}
            ...
            data.update({"type": "config", "action": "move", "where": where, "dst": dst, "xpath": xpath})
