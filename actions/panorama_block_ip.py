...
class BlockIp(BaseAction):
    def _add_address_entry(self, connector, action_result):
        connector.debug_print(f"Start adding address entry with param {self._param}")
        ...
        connector.debug_print(f"should_add_tag: {should_add_tag}")
        ...
        address_xpath = consts.IP_ADDR_XPATH.format(config_xpath=connector.util._get_config_xpath(self._param), ip_addr_name=name)
        data = {
            "type": "config",
            "action": "set",
            "key": connector.util._key,
            "xpath": address_xpath,
            "element": "{}{}".format(
                consts.IP_ADDR_ELEM.format(ip_type=ip_type, ip=block_ip), consts.IP_ADDR_TAG_ELEM.format(tag=tag) if should_add_tag else ""
            ),
        }
        connector.debug_print(f"Updating address entry with data: {data}")
    ...
    def execute(self, connector):
        ...
        if phantom.is_fail(status):
            return action_result.set_status(phantom.APP_ERROR, consts.PAN_ERROR_MESSAGE.format("blocking ip", action_result.get_message()))
