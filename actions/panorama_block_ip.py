...
class BlockIp(BaseAction):
    ...
    def _add_address_entry(self):
        self._connector.debug_print('Start adding address entry with param %s' % self._param)

        ip_type = None
        name = None
        tag = self._connector.get_container_id()
        block_ip = self._param[consts.PAN_JSON_IP]
        should_add_tag = self._param.get('should_add_tag', True)
        self._connector.debug_print('should_add_tag: %s' % should_add_tag)

        summary = {}

        # Add the tag to the system: Make this optional
        if should_add_tag:
            self._connector.debug_print('Adding tag...')
            data = {
                'type': 'config',
                'action': 'set',
                'key': self._connector.util._key,
                'xpath': consts.TAG_XPATH.format(config_xpath=self._connector.util._get_config_xpath(self._param)),
            ...
            if phantom.is_fail(status):
                self._action_result.update_summary({'add_address_entry': summary})
                return self._action_result.get_status(), name

            self._connector.debug_print('Done adding tag...')
        ...
        if block_ip.find('/') != -1:
            ...
            return (self._action_result.set_status(phantom.APP_ERROR, consts.PAN_ERROR_INVALID_IP_FORMAT), name)

        name = self._connector.util._get_addr_name(block_ip)
        ...
        self._connector.debug_print('Updating address entry with data: %s' % data)
        ...
        self._connector.debug_print('Done adding address entry with param')
        self._action_result.update_summary({'add_address_entry': summary})
        return phantom.APP_SUCCESS, name

    def execute(self):
        
        self._connector.debug_print('Start blocking ip')

        # Next create the ip
        self._connector.debug_print("Adding the IP Group")

        # Check where the IP should go
        use_source = self._param.get(consts.PAN_JSON_SOURCE_ADDRESS, consts.PAN_DEFAULT_SOURCE_ADDRESS)

        status, addr_name = self._add_address_entry()

        if phantom.is_fail(status):
            return self._action_result.set_status(
                phantom.APP_ERROR, consts.PAN_ERROR_MESSAGE.format("blocking ip", self._action_result.get_message())
            )
        ...
        data = {
            ...
            'action': 'set',
            'key': self._connector.util._key,
            'xpath': consts.ADDR_GRP_XPATH.format(config_xpath=self._connector.util._get_config_xpath(self._param), ip_group_name=ip_group_name),
            'element': consts.ADDR_GRP_ELEM.format(addr_name=addr_name)
        }

        status, response = self._connector.util._make_rest_call(data, self._action_result)
        self._action_result.update_summary({'add_ip_to_address_group': response})
        if phantom.is_fail(status):
            return self._action_result.set_status(
                phantom.APP_ERROR, consts.PAN_ERROR_MESSAGE.format("blocking ip", self._action_result.get_message()))
        ...
        if self._param.get('policy_name', ''):
            status = self._connector.util._update_security_policy(
                self._param, consts.SEC_POL_IP_TYPE, self._action_result, ip_group_name, use_source=use_source)
            if phantom.is_fail(status):
