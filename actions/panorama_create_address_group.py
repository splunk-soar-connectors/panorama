...
from panorama_consts import (
    ADD_GRP_TYPE_VAL_LIST,
    ADDR_GRP_XPATH,
    PAN_ERROR_MESSAGE,
    PAN_JSON_ADD_GRP_DIS_OVER,
    PAN_JSON_ADD_GRP_TYPE,
    PAN_JSON_DEVICE_GRP,
    param_mapping,
)
    ...
    def execute(self, connector):
        ...
        if parameter.get(PAN_JSON_ADD_GRP_DIS_OVER):
            if parameter.get(PAN_JSON_ADD_GRP_DIS_OVER).lower() not in ["yes", "no"]:
                return action_result.set_status(
                    phantom.APP_ERROR, "Please enter a valid value for 'disable-override' parameter from ['yes','no']"
                )
            else:
        ...
        if parameter.get(PAN_JSON_ADD_GRP_TYPE):
            if parameter.get(PAN_JSON_ADD_GRP_TYPE).lower() not in ADD_GRP_TYPE_VAL_LIST:
                return action_result.set_status(phantom.APP_ERROR, "Please enter a valid value for 'type' field as 'static' or 'dynamic'")
            else:
        ...
        if add_grp_present_status and connector.get_action_identifier() != "modify_address_group":
            return action_result.set_status(phantom.APP_ERROR, "An address group with this name already exists. Please use another name.")

        elif not add_grp_present_status and connector.get_action_identifier() == "modify_address_group":
            return action_result.set_status(phantom.APP_ERROR, "This address group is not present. Please enter a valid address group name.")
        ...
        if parameter.get("tag"):
            ...
            if tags:
                ...
                if phantom.is_fail(status):
                    return action_result.set_status(
                        phantom.APP_ERROR, PAN_ERROR_MESSAGE.format("Error occurred while creating the tags: ", message)
                    )
